...
function getDataMap() {
    fetch('./json/map_europe.json')
        .then(res => res.json())
        .then(data => {

            // injection (push) des données dans un tableaux
            let data2023 = [];

            data.forEach(function viewDataMap(map) {
                data2023.push(map.belgium.number, map.uk.number, map.netherlands.number, map.danemark.number, map.germany.number, map.swiss.number, map.france.number, map.portugal.number, map.espagne.number, map.italy.number, map.europeast.number);
            });

            // injection des données des tableaux dans les 11 balises correspondant aux 11 régions dans index.html
            for (let i = 0; i < 11; i++) {
                // M = millions
                document.querySelector(`.n${i} p`).innerHTML += data2023[i] + 'M';
            };
        });
}
